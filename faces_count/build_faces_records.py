...
from config import faces_config as config
from utilities.tfannotation import TFAnnotation
...
def main(_):
	...
	for row in rows[1:]:
		...
		row = row.split(",")
		label = "face"
		(imagePath, _, _, startX, startY, endX, endY) = row
		...
		p = os.path.sep.join([config.IMAGES_PATH, imagePath])
		b = D.get(p, [])
